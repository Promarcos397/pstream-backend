...
export async function scrapeVidSrcTo(tmdbId, type, season, episode) {
    try {
        ...
        const { data: mainPage } = await axios.get(`${vidSrcToBase}${embedPath}`, { headers, timeout: 8000 });
        const $ = cheerio.load(mainPage);
        const dataId = $('a[data-id]').attr('data-id');
        if (!dataId) return null;

        // Step 2: Get and Decrypt sources
        const { data: sourcesResp } = await axios.get(`${vidSrcToBase}/ajax/embed/episode/${dataId}/sources`, { headers });
        ...
        for (const source of sourcesResp.result) {
            ...
            if (source.title === 'Vidplay' || source.title === 'Vidstream') {
                const { data: sData } = await axios.get(`${vidSrcToBase}/ajax/embed/source/${source.id}`, { headers });
        ...
        const { data: futokenScript } = await axios.get(`${playerHost}/futoken`, { headers: playerHeaders });
        ...
        const { data: mediaInfo } = await axios.get(mediaUrl, { 
